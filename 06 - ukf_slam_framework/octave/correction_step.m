...
for i = 1:m
  ...
  z_points = [
    sqrt(pow2(landmarkXs - mu(1)) + pow2(landmarkYs - mu(2)));
    atan2(landmarkYs - mu(2), landmarkXs - mu(1)) - mu(3);
  ];
  ...
  zm = zeros(2, 1);
  zm(1) = sum(wm .* z_points(1, :));
  zm(2) = atan2(sum(wm .* sin(z_points(2, :))), sum(wm .* cos(z_points(2, :))));
%    z_points(1, :) * wm';
%    normalize_angle(atan2(sum(wm .* sin(z_points(2))), sum(wm .* cos(z_points(2)))))
%  ];
  ...
  z_actual_diff = z_actual - zm;
  z_actual_diff(2) = normalize_angle(z_actual_diff(2));
  mu += K*(z_actual_diff);
