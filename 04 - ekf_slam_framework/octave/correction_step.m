...
for i = 1:m
	...
	if(observedLandmarks(landmarkId)==false)
		...
		mu(3+2*landmarkId-1:3+2*landmarkId) = mu(1:2) + \
      [z(i).range * cos(z(i).bearing + mu(3));\
       z(i).range * sin(z(i).bearing + mu(3))];
  ...
  d = mu(3+2*landmarkId-1:3+2*landmarkId) - mu(1:2);
  q = d'*d;
  rq = sqrt(q);
  expectedZ(2*i-1:2*i) = [rq;atan2(d(2), d(1)) - mu(3)];

	% TODO: Compute the Jacobian Hi of the measurement function h for this observation
  H(2*i-1:2*i, [1:3, 3+2*landmarkId-1:3+2*landmarkId]) = \
    (1/q)*[ \
      -rq*d(1), -rq*d(2), 0, rq*d(1), rq*d(2);\
      d(2), -d(1), -q, -d(2), d(1)];
##  Hi = zeros(2, size(mu,1));
##  Hi(1:2, [1:3, 3+2*landmarkId-1:3+2*landmarkId]) = \
##    (1/q)*[ \
##      -rq*d(1), -rq*d(2), 0, rq*d(1), rq*d(2);\
##      d(2), -d(1), -q, -d(2), d(1)];
##	
...
sigma = (eye(size(K, 1), size(H, 2)) - K * H) * sigma;
